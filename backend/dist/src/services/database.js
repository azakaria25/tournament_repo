...
class DatabaseService {
    initialize() {
        ...
        weight INTEGER
                ...
                try {
                    yield sql `ALTER TABLE teams ADD COLUMN IF NOT EXISTS weight INTEGER`;
                }
                catch (error) {
        ...
        match_time TEXT
                ...
                try {
                    yield sql `ALTER TABLE matches ADD COLUMN IF NOT EXISTS court_number TEXT`;
                    yield sql `ALTER TABLE matches ADD COLUMN IF NOT EXISTS match_time TEXT`;
                }
                catch (error) {
        ...
        pin TEXT
                ...
                try {
                    yield sql `ALTER TABLE tournaments ADD COLUMN IF NOT EXISTS pin TEXT`;
                }
                catch (error) {
                    // Column might already exist, ignore error
    ...
    `;
            ...
            const matches = yield this.getMatches();
            return Object.assign(Object.assign({}, tournament), { pin: tournament.pin || '', teams: mappedTeams, matches: matches.filter(m => m.tournamentId === id) });
        });
    }
    ...
    deleteAllTournaments() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield sql `DELETE FROM tournament_teams`;
            yield sql `DELETE FROM matches`;
            yield sql `DELETE FROM teams`;
            yield sql `DELETE FROM tournaments`;
        });
    }
}
